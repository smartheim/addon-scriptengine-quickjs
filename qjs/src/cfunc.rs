...
impl ContextRef {
    ...
    pub fn new_c_function<T: NewValue,F:Fn(&ContextRef, Option<&Value>, &[Value]) -> T>(
        &self,
        func: F,
        name: Option<&str>,
        length: usize,
    ) -> Result<Local<Value>, Error> {
        unsafe extern "C" fn stub<T: NewValue,F:Fn(&ContextRef, Option<&Value>, &[Value]) -> T>(
            ctx: *mut ffi::JSContext,
            this_val: ffi::JSValue,
            argc: c_int,
            argv: *mut ffi::JSValue,
            ...
            panic::catch_unwind(|| {
                ...
                let func = ctxt.get_userdata_unchecked::<F>(data.cast().as_ref());
        ...
        let func = self.new_c_function_data(stub::<T,F>, length, 0, self.new_userdata(func))?;
...
macro_rules! new_func_value {
    ...
    ($($Arg:ident)+) => {
        impl<Ret: NewValue, $($Arg : ExtractValue),*> NewValue for fn(&ContextRef, $( $Arg ),*) -> Ret {
            fn new_value(self, ctxt: &ContextRef) -> ffi::JSValue {
                unsafe extern "C" fn stub<Ret: NewValue, $($Arg : ExtractValue),*>(
                    ...
                    panic::catch_unwind(|| {
                        let ctxt = ContextRef::from_ptr(ctx);
                        let data = ptr::NonNull::new_unchecked(data);
                        let func = ctxt.get_userdata_unchecked::<fn(&ContextRef, $( $Arg ),*) -> Ret>(data.cast().as_ref());
                        let func = *func.as_ref();
                        let args = slice::from_raw_parts(argv, argc as usize);
...
mod tests {
    ...
    fn new_value() {
        ...
        let rt = Runtime::new();
        let _ctxt = Context::new(&rt);

        let _hello: fn(String) -> String = hello;
