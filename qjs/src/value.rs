...
impl fmt::Display for Local<'_, Value> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.write_str(&self.to_cstring().unwrap())
    }
}
...
impl<'a> Local<'a, Value> {
    ...
    pub fn to_cstring(&self) -> Option<String> {
...
impl ContextRef {
    ...
    pub fn to_cstring(&self, val: &Value) -> Option<String> {
        let mut len = 0;
...
impl ExtractValue for Value {
    fn extract_value(v: &Local<Value>) -> Option<Self> {
        // This is not safe. The context lifetime is not guaranteed to be valid.
        // This will only be called by rust-to-js functions and context not never outlive the function scope.
        v.clone().inner.take()
    }
}
